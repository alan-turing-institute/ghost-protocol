...
the image plane; and a ``focal length,'' $f$. The focal length has
units of ``pixels per metre'' and is the size, in the camera frame, of
an object of size \qty{1}{\metre} at a distance of~\qty{1}{\metre} in
the world frame.
...
\begin{equation*}
  \begin{split}
    y &= \vec{x}\cdot(\hat{\vec{u}}\times\hat{\vec{n}})f/d. \\
  \end{split}
\end{equation*}
\end{enumerate}
