...
VideoFilterRunnable::VideoFilterRunnable(VideoFilter *filter) : m_filter(filter)
{
    connect(this, &VideoFilterRunnable::stringFound, m_filter, &VideoFilter::setResult, Qt::QueuedConnection);
    m_timer.start();
    // analyze empty frame to establish D-Bus connection
    analyze(QDBusUnixFileDescriptor(0), 0, QVideoSurfaceFormat(QSize(0,0), QVideoFrame::PixelFormat::Format_BGRA32));
}
...
QVideoFrame VideoFilterRunnable::run(QVideoFrame *input, const QVideoSurfaceFormat &surfaceFormat, QVideoFilterRunnable::RunFlags flags)
{
    ...
    if (m_reply.isError()) {
        qWarning() << "Video filter error" << m_reply.error().message();
    } else {
...
void VideoFilterRunnable::analyze(QDBusUnixFileDescriptor fd, uint bufferSize, QVideoSurfaceFormat surfaceFormat)
{
    QDBusMessage message = QDBusMessage::createMethodCall("uk.ac.turing.stream", "/uk/ac/turing/stream", "uk.ac.turing.stream", "decodeFromDescriptor");
    QList<QVariant> args;
